...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] CrateNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is CrateNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (CrateNetworkObject.Length > 0 && CrateNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(CrateNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<CrateBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateCrate instead, its shorter and easier to type out ;)")]
		public CrateBehavior InstantiateCrateNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(CrateNetworkObject[index]);
			var netBehavior = go.GetComponent<CrateBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<CrateBehavior>().networkObject = (CrateNetworkObject)obj;
		...
		/// <summary>
		/// Instantiate an instance of Crate
		/// </summary>
		/// <returns>
		/// A local instance of CrateBehavior
		/// </returns>
		/// <param name="index">The index of the Crate prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public CrateBehavior InstantiateCrate(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(CrateNetworkObject[index]);
			var netBehavior = go.GetComponent<CrateBehavior>();
